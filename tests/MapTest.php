...
class MapTest extends TestCase
{
    ...
    public function test_minimal_config()
    {
        ...
        $this->assertSame(
            'const map0={const map=L.map("map0",{center:L.latLng(0,0),zoom:10});} map0.map',
            $map->getJs()
        );
    }
    ...
    public function test_map()
    {
        ...
        $this->assertSame(
            'const map1={const layer0=L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{maxZoom:19,attribution:"&copy; <a href=\"https://www.openstreetmap.org/copyright\">OpenStreetMap</a> contributors"});const map=L.map("map1",{center:L.latLng(51.77255,-4.95325),layers:[layer0],zoom:12});const layer1=L.layerGroup([L.circle(L.latLng(51.77255,-4.95325),{radius:15000,color:"#20ffcd"}).bindTooltip("15km radius"),L.circle(L.latLng(51.77255,-4.95325),{radius:10000,color:"#3388ff"}).bindTooltip("10km radius"),L.circle(L.latLng(51.77255,-4.95325),{radius:5000,color:"#573CFF"}).bindTooltip("5km radius"),L.marker(L.latLng(51.77255,-4.95325),{icon:L.icon({iconAnchor:L.point(12,40),iconUrl:"leaflet/images/marker-icon.png",shadowUrl:"leaflet/images/marker-shadow.png"})}).bindPopup("<p><b>Little Dumpledale Farm</b></p><p>Ashdale Lane<br>Sardis<br>Haverfordwest<br>Pembrokeshire<br>SA62 4NT</p><p>Tel: +44 1646 602754</p>")]).addTo(this.map);const layer2=L.layerGroup([L.marker(L.latLng(51.749151,-4.913822),{icon:L.icon({iconAnchor:L.point(12,40),iconUrl:"leaflet/images/marker-icon.png",shadowUrl:"leaflet/images/marker-shadow.png"})}).bindPopup("<p><b>The Cottage Inn</b></p><p>Llangwm<br>Haverfordwest<br>Pembrokeshire<br>SA62 4HH</p><p>Tel: +44 1437 891494</p>"),L.marker(L.latLng(51.7079864,-4.925951),{icon:L.icon({iconAnchor:L.point(12,40),iconUrl:"leaflet/images/marker-icon.png",shadowUrl:"leaflet/images/marker-shadow.png"})}).bindPopup("<p><b>Jolly Sailor</b></p><p>Burton<br>Milford Haven<br>Pembrokeshire<br>SA73 1NX</p><p>Tel: +44 1646 600378</p>")]);const layer3=L.marker(L.latLng(51.786979,-4.977206),{draggable:true,icon:L.icon({iconAnchor:L.point(12,40),iconUrl:"leaflet/images/marker-icon.png",shadowUrl:"leaflet/images/marker-shadow.png"})}).bindPopup("Drag me and see what happens").on("dragend",function(e){const position=e.target.getLatLng();window.alert("Moved by " + Math.floor(e.distance) + " pixels\nNew position " + position.lat + ", " + position.lng);});const control0=L.control.layers(null,{"Little Dumpledale":layer1,"Pubs":layer2,"Draggable":layer3}).addTo(this.map);const control1=L.control.scale().addTo(this.map);} map1.map',
            $map->getJs()
        );
    }
}
